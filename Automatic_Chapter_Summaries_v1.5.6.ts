 ...
 * v1.5.6: Using list() instead of has() due to NovelAI API bug where has() returns false
 *         even when permissions are granted
 */
async function checkPermissions(): Promise<boolean> {
    try {
        // Use list() and manually check - workaround for has() API bug
        ...
        if (DEBUG_MODE) {
            ...
            api.v1.log(`Current permissions: ${JSON.stringify(currentPerms)}`);
        }
...
async function ensurePermissions(operation: string): Promise<boolean> {
    ...
    if (!hasRequiredPermissions) {
        api.v1.error(`Cannot ${operation}: Missing required permissions. Please grant lorebookEdit and documentEdit permissions for this script.`);
        return false;
    }
...
/**
 * Build the status display text content
 * v1.5.6: Extracted to allow building content before panel registration
 */
async function buildStatusDisplayText(): Promise<string> {
    ...
    // Count condensed ranges properly
    ...
    // Show background work indicator
 ...
 * MODIFIED in v1.5.6: Added permission/initialization check, uses buildStatusDisplayText helper
 */
async function updateStatusPanel(): Promise<void> {
    ...
    // This handles the case where NovelAI's permission system isn't ready during early init
    if (!hasRequiredPermissions || !lorebookCategoryId) {
        ...
        for (let attempt = 0; attempt < 3; attempt++) {
            ...
            // Wait before retry (increasing delay: 200ms, 400ms, 600ms)
            await api.v1.timers.sleep(200 * (attempt + 1));
        }
    }

    // If still no permissions or category after retries, show waiting message
    if (!hasRequiredPermissions || !lorebookCategoryId) {
        try {
            await api.v1.ui.updateParts([
                {
                    id: "status-display-box",
                    content: [{
                        type: "text",
                        text: "**Waiting for permissions...**\n\nPlease grant lorebookEdit and documentEdit permissions when prompted.",
                        markdown: true
                    }]
                },
                {
                    ...
                    content: [{
                        type: "text",
                        text: "_Waiting for initialization..._",
                        markdown: true,
                        style: { fontStyle: "italic", color: "rgba(255, 255, 255, 0.5)" }
                    }]
                },
            ...
            if (DEBUG_MODE) {
                const errorMsg = error instanceof Error ? error.message : String(error);
                api.v1.log("Could not update status panel (waiting state):", errorMsg);
            }
        }
        return;
    }

    // v1.5.6: Use helper function to build status text
    const statusText = await buildStatusDisplayText();
    ...
    try {
        // Update status display box (v1.5.6: use box content pattern like other boxes)
        await api.v1.ui.updateParts([{
            id: "status-display-box",
            content: [{
                type: "text",
...
(async () => {
    ...
    // v1.5.6: Check permissions with retry to handle timing issues during startup
    // NovelAI's permission system may not be ready immediately when script loads
    for (let attempt = 0; attempt < 3; attempt++) {
        await checkPermissions();
        if (hasRequiredPermissions) {
            await initializeLorebookCategory();
        ...
        // Wait before retry (increasing delay: 300ms, 600ms, 900ms)
        await api.v1.timers.sleep(300 * (attempt + 1));
    }

    if (!hasRequiredPermissions) {
        api.v1.log("⚠️ Missing required permissions. Please grant lorebookEdit and documentEdit permissions for this script.");
    }

    // v1.5.6: Build real content BEFORE panel registration to avoid "Loading..." stuck state
    // The updateParts API may silently fail during initialization, so we register with real data
    let initialStatusText = "**Initializing...**\n\nPlease wait while we load your chapter summary data.";
    let initialChangedChaptersContent: UIPart[] = [{
        type: "text",
        text: "_Checking for changed chapters..._",
        markdown: true,
        style: { fontStyle: "italic", color: "rgba(255, 255, 255, 0.5)" }
    }];
    let initialCondensedRangesContent: UIPart[] = [{
        type: "text",
        text: "_Loading condensed ranges..._",
        markdown: true,
        style: { fontStyle: "italic", color: "rgba(255, 255, 255, 0.5)" }
    }];
    ...
    if (hasRequiredPermissions && lorebookCategoryId) {
        try {
            ...
            if (DEBUG_MODE) {
                api.v1.log("Built initial panel content successfully");
            }
        } catch (error) {
            const errorMsg = error instanceof Error ? error.message : String(error);
            api.v1.log("Could not build initial panel content:", errorMsg);
            // Fall back to placeholder content (already set above)
        }
    }
    ...
    const panelContent: UIPart[] = [
            ...
            // Changed chapters status box (dynamic content)
            {
                type: "box",
                id: "changed-chapters-box",
                content: initialChangedChaptersContent,
                style: {
                    padding: "12px",
            ...
            // v1.5.1: Condensed ranges status box (dynamic content)
            {
                type: "box",
                id: "condensed-ranges-box",
                content: initialCondensedRangesContent,
                style: {
                    padding: "12px",
    ...
    // Register UI Panel with pre-built content
    ...
    if (DEBUG_MODE) {
        api.v1.log("Panel registered with initial content");
    }

    // v1.5.6: Since we built content before registration, just clear any leftover progress indicators
    // No need for retry loop - the panel already has real data
    try {
        await api.v1.ui.updateParts([{
            id: "rebuild-progress",
            text: ""
        ...
        // Ignore - panel may not support this element yet
    }
